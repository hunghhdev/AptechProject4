<template>
  <div class="app-container">
    ...
    <el-tabs type="border-card" @tab-click="handleSelectTab">
      <el-tab-pane :label="$t('bookings.tabBooking')">
        ...
        >
          ...
          <el-table-column :label="$t('bookings.hourlyPrice')" min-width="150" align="center">
      ...
      <el-tab-pane :label="$t('bookings.tabBooked')">
        ...
        >
          ...
          <el-table-column :label="$t('bookings.nameCus')" min-width="150" align="center">
            <template slot-scope="scope">{{ scope.row.customerName }}</template>
          </el-table-column>
          <el-table-column :label="$t('bookings.phoneCus')" min-width="150" align="center">
          ...
          <el-table-column :label="$t('bookings.fromDate')" min-width="150" align="center">
            <template slot-scope="scope">{{ scope.row.fromDate }}</template>
          </el-table-column>
          <el-table-column :label="$t('bookings.toDate')" min-width="150" align="center">
            <template slot-scope="scope">{{ scope.row.toDate }}</template>
          </el-table-column>
          <el-table-column fixed="right" :label="$t('common.action')" align="center" min-width="300">
            <template slot-scope="{row}">
              <el-button icon="el-icon-close" type="danger" size="mini" @click="handleBooking(row)">
                {{ $t("common.btnCancel") }}
              </el-button>
              <el-button icon="el-icon-check" type="primary" size="mini" @click="handleBooking(row)">
    ...
    <el-dialog :title="$t('bookings.title')" :visible.sync="dialogBookingVisible">
      <el-form ref="dataForm" :model="tempData" label-position="left" label-width="30%" style="width: 90%; margin-left:30px;" :rules="rules">
        ...
        <el-form-item :label="$t('bookings.type')" prop="price">
          <el-switch v-model="tempData.type" :active-text="$t('bookings.hour')" :inactive-text="$t('bookings.day')" disabled />
        </el-form-item>
        ...
        <el-form-item :label="$t('bookings.phoneCus')" prop="phoneCus">
          <el-autocomplete v-model="tempData.phoneCus" :fetch-suggestions="querySearch" :trigger-on-focus="false"
            @select="handleSelect" class="filter-item" style="width: 475px" >
            <table slot-scope="{ item }" width="475px">
              <tr>
                <th>{{ $t('bookings.phoneCus') }}</th> |
                <th>{{ $t('bookings.nameCus') }}</th> |
...
<script>
  ...
  import {listBooked} from "../../api/booking";
  export default {
    name: "bookings",
    ...
    methods: {
      ...
      handleSelect(item) {
        if (item) {
          ...
          this.tempData.phoneCus = item.phone;
      ...
      handleSelectTab(tab) {
        if (tab.index==0){
          ...
          console.log("Có gẻì đấy sai sai")
        }
      },
